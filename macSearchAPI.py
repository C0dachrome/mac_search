...
def start_target(channel, bssid):
    ...
    try:
        # 1. Kill any current airodump processes
        subprocess.run(['sudo', 'killall', 'airodump-ng'], stderr=subprocess.DEVNULL)
        
        # 2. Build the targeted command
        # Using '-c' for channel and '--bssid' for the specific target
        cmd = [
            'sudo', 'airodump-ng',
            '-c', str(channel),
            '--bssid', str(bssid),
        ...
        return jsonify({"status": "success", "message": f"Targeting {bssid} on CH {channel}"})
